...
exports.generateModels = exports.codegen = exports.processFields = exports.generateJsonInterfaces = exports.renderTemplate = void 0;
...
async function generateJsonInterfaces(projectPath, schema) {
    ...
    const jsonObjects = common_1.getAllJsonObjects(schema);
    const jsonInterfaces = jsonObjects.map((r) => {
        const object = common_1.setJsonObjectType(r, jsonObjects);
...
function processFields(type, className, fields, indexFields = []) {
    ...
    for (const field of fields) {
        ...
        switch (field.type) {
            default: {
                injectField.type = types_mapping_1.transformTypes(className, field.type.toString());
                if (!injectField.type) {
            ...
            case 'Json': {
                ...
                injectField.type = lodash_1.upperFirst(field.jsonInterface.name);
...
async function prepareDirPath(path, recreate) {
    try {
        await util_1.promisify(rimraf_1.default)(path);
        if (recreate) {
...
async function codegen(projectPath) {
    ...
    const manifest = common_1.loadProjectManifest(projectPath);
...
async function generateModels(projectPath, schema) {
    const extractEntities = common_1.getAllEntitiesRelations(schema);
    for (const entity of extractEntities.models) {
        const baseFolderPath = '.../../base';
        const className = lodash_1.upperFirst(entity.name);
