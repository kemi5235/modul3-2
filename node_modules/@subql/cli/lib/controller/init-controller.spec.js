...
describe('Cli can create project (mocked)', () => {
    it('throw error when git clone failed', async () => {
        ...
        simple_git_1.default().clone.mockImplementationOnce((path, cb) => cb(new Error()));
        await expect(init_controller_1.createProject(tempPath, projectSpec)).rejects.toThrow(/Failed to clone starter template from git/);
    });
});
