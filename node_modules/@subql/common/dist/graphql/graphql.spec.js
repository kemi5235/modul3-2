...
describe('utils that handle schema.graphql', () => {
    it('support @entity annotation in the schema', () => {
        const graphqlSchema = graphql_tag_1.default `
    ...
    `;
        const schema = schema_1.buildSchemaFromDocumentNode(graphqlSchema);
    ...
    `;
        const schema = schema_1.buildSchemaFromDocumentNode(graphqlSchema);
        const entities = entities_1.getAllEntitiesRelations(schema);
    ...
    `;
        expect(() => schema_1.buildSchemaFromDocumentNode(graphqlSchema)).toThrow();
    });
    ...
    `;
        const schema = schema_1.buildSchemaFromDocumentNode(graphqlSchema);
        expect(() => entities_1.getAllEntitiesRelations(schema)).toThrow('Please check entity Account with field transfers has correct relation with entity Transfer');
    });
    ...
    `;
        const schema = schema_1.buildSchemaFromDocumentNode(graphqlSchema);
        expect(() => entities_1.getAllEntitiesRelations(schema)).toThrow(/^index can not be added on field id/);
    });
    ...
    `;
        ...
        expect((_c = entities.models) === null || _c === void 0 ? void 0 : _c[0].indexes[1].unique).toBe(true);
    });
    ...
    `;
        const schema = schema_1.buildSchemaFromDocumentNode(graphqlSchema);
        const accountModel = entities_1.getAllEntitiesRelations(schema).models.find((model) => model.name === 'Account');
