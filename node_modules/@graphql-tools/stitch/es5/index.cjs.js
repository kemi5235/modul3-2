...
function validateFieldConsistency(finalFieldConfig, candidates, typeMergingOptions) {
    ...
    var finalFieldNull = graphql.isNonNullType(finalFieldConfig.type);
    ...
    if (getValidationSettings(fieldNamespace, typeMergingOptions).strictNullComparison &&
        candidates.some(function (c) { return finalFieldNull !== graphql.isNonNullType(c.fieldConfig.type); })) {
    ...
    else if (finalFieldNull && candidates.some(function (c) { return !graphql.isNonNullType(c.fieldConfig.type); })) {
    ...
    candidates.forEach(function (_a) {
        ...
        Object.entries(fieldConfig.args).forEach(function (_a) {
            var _b = tslib.__read(_a, 2), argName = _b[0], arg = _b[1];
    ...
    Object.entries(argCandidatesMap).forEach(function (_a) {
        var _b = tslib.__read(_a, 2), argName = _b[0], argCandidates = _b[1];
        ...
        var finalArgType = graphql.getNamedType(finalArgConfig.type);
        var finalArgNull = graphql.isNonNullType(finalArgConfig.type);
        ...
        if (getValidationSettings(argNamespace, typeMergingOptions).strictNullComparison &&
            argCandidates.some(function (c) { return finalArgNull !== graphql.isNonNullType(c.type); })) {
        ...
        else if (!finalArgNull && argCandidates.some(function (c) { return graphql.isNonNullType(c.type); })) {
...
function validateInputObjectConsistency(fieldInclusionMap, candidates, typeMergingOptions) {
    Object.entries(fieldInclusionMap).forEach(function (_a) {
        var _b = tslib.__read(_a, 2), fieldName = _b[0], count = _b[1];
...
function validateInputFieldConsistency(finalInputFieldConfig, candidates, typeMergingOptions) {
    ...
    var finalInputFieldType = graphql.getNamedType(finalInputFieldConfig.type);
    var finalInputFieldNull = graphql.isNonNullType(finalInputFieldConfig.type);
    ...
    if (getValidationSettings(inputFieldNamespace, typeMergingOptions).strictNullComparison &&
        candidates.some(function (c) { return finalInputFieldNull !== graphql.isNonNullType(c.inputFieldConfig.type); })) {
    ...
    else if (!finalInputFieldNull && candidates.some(function (c) { return graphql.isNonNullType(c.inputFieldConfig.type); })) {
...
function validateTypeConsistency(finalElementConfig, candidates, definitionType, settingNamespace, typeMergingOptions) {
    var finalNamedType = graphql.getNamedType(finalElementConfig.type);
    var finalIsScalar = graphql.isScalarType(finalNamedType);
    ...
    candidates.forEach(function (c) {
        ...
        var currentNamedType = graphql.getNamedType(c.type);
        if (finalNamedType.toString() !== currentNamedType.toString()) {
            ...
            var bothScalars = finalIsScalar && graphql.isScalarType(currentNamedType);
...
function validateInputEnumConsistency(inputEnumType, candidates, typeMergingOptions) {
    ...
    candidates.forEach(function (candidate) {
        var enumType = graphql.getNamedType(candidate.type);
...
function getValidationSettings(settingNamespace, typeMergingOptions) {
    var _a, _b, _c;
    return tslib.__assign(tslib.__assign({}, ((_a = typeMergingOptions === null || typeMergingOptions === void 0 ? void 0 : typeMergingOptions.validationSettings) !== null && _a !== void 0 ? _a : {})), ((_c = (_b = typeMergingOptions === null || typeMergingOptions === void 0 ? void 0 : typeMergingOptions.validationScopes) === null || _b === void 0 ? void 0 : _b[settingNamespace]) !== null && _c !== void 0 ? _c : {}));
}
...
function mergeObjectTypeCandidates(typeName, candidates, typeMergingOptions) {
    ...
    if (astNodes.length > 1 && fieldAstNodes.length) {
        astNodes.push(tslib.__assign(tslib.__assign({}, astNodes[astNodes.length - 1]), { fields: JSON.parse(JSON.stringify(fieldAstNodes)) }));
    }
    var astNode = astNodes
        .slice(1)
        .reduce(function (acc, astNode) {
        return merge.mergeType(astNode, acc, { ignoreFieldConflicts: true });
    }, astNodes[0]);
    var extensionASTNodes = [].concat(pluck('extensionASTNodes', candidates));
    var extensions = Object.assign.apply(Object, tslib.__spreadArray([{}], tslib.__read(pluck('extensions', candidates))));
    var typeConfig = {
        name: typeName,
...
function mergeInputObjectTypeCandidates(typeName, candidates, typeMergingOptions) {
    ...
    var astNode = astNodes.slice(1).reduce(function (acc, astNode) {
        return merge.mergeInputType(astNode, acc, {
...
function mergeInterfaceTypeCandidates(typeName, candidates, typeMergingOptions) {
    ...
    var astNode = astNodes.slice(1).reduce(function (acc, astNode) {
        return merge.mergeInterface(astNode, acc, {
            ignoreFieldConflicts: true,
        });
    }, astNodes[0]);
    var extensionASTNodes = [].concat(pluck('extensionASTNodes', candidates));
...
function mergeEnumTypeCandidates(typeName, candidates, typeMergingOptions) {
    ...
    var astNode = astNodes
        .slice(1)
        .reduce(function (acc, astNode) {
        return merge.mergeEnum(astNode, acc, { consistentEnumMerge: true });
    }, astNodes[0]);
    var extensionASTNodes = [].concat(pluck('extensionASTNodes', candidates));
    var extensions = Object.assign.apply(Object, tslib.__spreadArray([{}], tslib.__read(pluck('extensions', candidates))));
    var typeConfig = {
...
function defaultEnumValueConfigMerger(candidates) {
    var preferred = candidates.find(function (_a) {
        ...
        return delegate.isSubschemaConfig(transformedSubschema) && ((_c = (_b = transformedSubschema.merge) === null || _b === void 0 ? void 0 : _b[type.name]) === null || _c === void 0 ? void 0 : _c.canonical);
    });
...
function mergeScalarTypeCandidates(typeName, candidates, typeMergingOptions) {
    ...
    var astNode = astNodes
        .slice(1)
        .reduce(function (acc, astNode) { return merge.mergeScalar(astNode, acc); }, astNodes[0]);
    var extensionASTNodes = [].concat(pluck('extensionASTNodes', candidates));
    var extensions = Object.assign.apply(Object, tslib.__spreadArray([{}], tslib.__read(pluck('extensions', candidates))));
    var typeConfig = {
        name: typeName,
...
function defaultTypeCandidateMerger(candidates) {
    var canonical = candidates.filter(function (_a) {
        var _b, _c;
        var type = _a.type, transformedSubschema = _a.transformedSubschema;
        return delegate.isSubschemaConfig(transformedSubschema) ? (_c = (_b = transformedSubschema.merge) === null || _b === void 0 ? void 0 : _b[type.name]) === null || _c === void 0 ? void 0 : _c.canonical : false;
    });
    ...
    candidates.forEach(function (_a) {
        ...
        if (delegate.isSubschemaConfig(transformedSubschema) &&
            ...
            Object.entries(transformedSubschema.merge[type.name].fields).forEach(function (_a) {
                var _b = tslib.__read(_a, 2), fieldName = _b[0], mergedFieldConfig = _b[1];
...
function isolateComputedFieldsTransformer(subschemaConfig) {
    ...
    Object.entries(subschemaConfig.merge).forEach(function (_a) {
        var _b;
        var _c = tslib.__read(_a, 2), typeName = _c[0], mergedTypeConfig = _c[1];
        ...
        if (mergedTypeConfig.computedFields) {
            ...
            Object.entries(mergedTypeConfig.computedFields).forEach(function (_a) {
                var _b;
                var _c = tslib.__read(_a, 2), fieldName = _c[0], mergedFieldConfig = _c[1];
                ...
                mergedTypeConfig.fields[fieldName] = tslib.__assign(tslib.__assign(tslib.__assign({}, ((_b = mergedTypeConfig.fields[fieldName]) !== null && _b !== void 0 ? _b : {})), mergedFieldConfig), { computed: true });
            });
        ...
        if (mergedTypeConfig.fields) {
            ...
            if (isolatedFieldCount && isolatedFieldCount !== Object.keys(objectType.getFields()).length) {
                baseSchemaTypes[typeName] = tslib.__assign(tslib.__assign({}, mergedTypeConfig), { fields: baseFields_1 });
                isolatedSchemaTypes[typeName] = tslib.__assign(tslib.__assign({}, mergedTypeConfig), { fields: isolatedFields_1, canonical: undefined });
            }
        }
    });
...
function splitMergedTypeEntryPointsTransformer(subschemaConfig) {
    ...
    var _loop_1 = function (i) {
        var subschemaPermutation = delegate.cloneSubschemaConfig(subschemaConfig);
...
function applySubschemaConfigTransforms(subschemaConfigTransforms, subschemaOrSubschemaConfig, subschemaMap, originalSubschemaMap) {
    ...
    var transformedSubschemas = transformedSubschemaConfigs.map(function (ssConfig) { return new delegate.Subschema(ssConfig); });
